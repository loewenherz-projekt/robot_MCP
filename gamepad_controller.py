...
class XboxGamepadController:
    ...
    def start(self) -> None:
        ...
        print("Triggers:      Base strafe left/right")
        print("D-Pad:         Arm up/down")
        print("Right stick:   Arm forward/back & rotate arm")
        print("X/Y buttons:   Rotate gripper left/right")
        print("A/B buttons:   Open/Close gripper")
    ...
    def _handle_inputs(self) -> None:
        ...
        up_delta = hat_y * self.spatial_step_mm
        ...
        # Build full action with current arm positions and desired base velocities
        action = self.robot._build_action(self.robot.positions_deg)
        action["x.vel"] = -ly * self.base_speed_scale
        action["y.vel"] = (lt - rt) * self.base_speed_scale
        action["theta.vel"] = -lx * self.base_rot_speed_deg
